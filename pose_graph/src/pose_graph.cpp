...
void PoseGraph::addKeyFrame(KeyFrame* cur_kf, bool flag_detect_loop)
{
	...
	if (loop_index != -1)
	{
        ...
        if (cur_kf->findConnection(old_kf))
        {
            ...
            old_kf->getVioPose(w_P_old, w_R_old); //Pw1_i_old, Rw1_i_old (old代表回环帧)
            cur_kf->getVioPose(vio_P_cur, vio_R_cur); //Pw2_i_cur, Rw2_i_old

            //获取当前帧与回环帧的相对位姿relative_q、relative_t
            Vector3d relative_t;
            Quaterniond relative_q;
            relative_t = cur_kf->getLoopRelativeT();//Pold_cur
            relative_q = (cur_kf->getLoopRelativeQ()).toRotationMatrix();//Rold_cur

            //重新计算当前帧位姿w_P_cur、w_R_cur
            w_P_cur = w_R_old * relative_t + w_P_old;
            w_R_cur = w_R_old * relative_q;//Rw1_i_cur = Rw1_i_old * Rold_cur
            
            //由于当前帧和闭环帧的世界系并不相同，滑动窗口所在的世界系为w2,为原世界系w1产生累积误差后的结果，因此在将每一帧包装成
            //Keyframe的时候，其所在的世界系为当时所在滑动窗口的世界系。
            //通过PnP，实际上求得了回环帧i和当前帧j的相对变换矩阵，即Ti->j,加上本身知道Tw1->i和Tw2-j，因此可以计算累积偏移位姿：
            //1.先得到当前帧相对于w1世界系下的位姿：Rw1_i_cur, Pw1_i_cur
            //2.得到偏移位姿：Tw1->w2 = Tw1_i_curr * (Tw2_i_curr)^{-1}　　(hansry)
            double shift_yaw;
            Matrix3d shift_r;
            Vector3d shift_t; 
            ...
            //将所有当前关键帧对应的图像序列的位姿都移动到世界坐标系w1下
            //由于在前面已经得到了俩个sequence坐标系w1(回环关键帧)和w2(当前关键帧)的偏移，因此将keyframelist中的帧重新定位到w1世界坐标系中
            if (old_kf->sequence != cur_kf->sequence && sequence_loop[cur_kf->sequence] == 0)
            {  
                w_r_vio = shift_r;//Rw1->w2
                w_t_vio = shift_t;//tw1->w2
                vio_P_cur = w_r_vio * vio_P_cur + w_t_vio;
                vio_R_cur = w_r_vio *  vio_R_cur; 
                cur_kf->updateVioPose(vio_P_cur, vio_R_cur);
...
void PoseGraph::optimize4DoF()
{
    while(true)
    {
        ...
        //取出最新一个待优化帧（其中m_optimize_buf存储的都是找到了闭环帧的关键帧）作为当前帧
        m_optimize_buf.lock();
        ...
        if (cur_index != -1)
        {
            ...
            m_keyframelist.lock();
            KeyFrame* cur_kf = getKeyFrame(cur_index);//从keyframelist中找到loop_index的关键帧

            int max_length = cur_index + 1;

            // w^t_i   w^q_i
            //max_length表示最大的优化变量个数，需要优化的关键帧为从第一次回环帧到当前帧间的所有关键帧
            ...
            for (it = keyframelist.begin(); it != keyframelist.end(); it++)
            {
                ...
                tmp_q = tmp_r;
                t_array[i][0] = tmp_t(0); //由于ceres处理的是数组，因此需要将各种Matrix和Vector转换为数组 (hansry)
                t_array[i][1] = tmp_t(1);
                t_array[i][2] = tmp_t(2);
                q_array[i] = tmp_q;
                ...
                problem.AddParameterBlock(euler_array[i], 1, angle_local_parameterization); //将关键帧位姿设为优化变量
                problem.AddParameterBlock(t_array[i], 3);

                if ((*it)->index == first_looped_index || (*it)->sequence == 0)
                {   
                    problem.SetParameterBlockConstant(euler_array[i]);//固定第一个回环关键帧的位姿参数（hansry）
                    problem.SetParameterBlockConstant(t_array[i]);
                }

                //添加序列边 (Sequential edge)
                for (int j = 1; j < 5; j++)
                {
                  //cout << "pose_graph 544 sequence_array[i]: " << sequence_array[i] << endl;
                  //cout << "pose_graph 545 sequence_array[i-j]: " << sequence_array[i-j] << endl;
                  if (i - j >= 0 && sequence_array[i] == sequence_array[i-j])
                  {
                    Vector3d euler_conncected = Utility::R2ypr(q_array[i-j].toRotationMatrix());
                    Vector3d relative_t(t_array[i][0] - t_array[i-j][0], t_array[i][1] - t_array[i-j][1], t_array[i][2] - t_array[i-j][2]);
                    relative_t = q_array[i-j].inverse() * relative_t; //p{i-j->i} = R{w->i-j}^{-1}*(p{w->i}-p{w->i-j})
                    double relative_yaw = euler_array[i][0] - euler_array[i-j][0]; //yaw{i-j->i} = yaw{i}-yaw{i-j}
            ...
            */
            ...
            m_keyframelist.lock();
            i = 0;
            //将经过pose_graph优化后的帧重新由数组转为向量或者矩阵 (hansry)
            for (it = keyframelist.begin(); it != keyframelist.end(); it++)
            {
                if ((*it)->index < first_looped_index)
                    continue;
            ...
            m_drift.lock();
            //cur_r指经过优化后的当前帧的位姿（基于世界系w1）,vio_r指之前计算出来的当前帧的位姿（基于世界系w2）
            yaw_drift = Utility::R2ypr(cur_r).x() - Utility::R2ypr(vio_r).x();
            r_drift = Utility::ypr2R(Vector3d(yaw_drift, 0, 0)); //Rw1->w2
            t_drift = cur_t - r_drift * vio_t; //tw1->w2
            m_drift.unlock();
            ...
            it++;
            //将keyframelist中当前帧后面的关键帧通过求解的偏移量转换到一致world坐标系(w1)下
            for (; it != keyframelist.end(); it++)
            {
                Vector3d P;
                Matrix3d R;
...
void PoseGraph::updateKeyFrameLoop(int index, Eigen::Matrix<double, 8, 1 > &_loop_info)
{
    ...
    if (abs(_loop_info(7)) < 30.0 && Vector3d(_loop_info(0), _loop_info(1), _loop_info(2)).norm() < 20.0)
    {
        if (FAST_RELOCALIZATION)
        {
            ...
            Vector3d relative_t;
            Quaterniond relative_q;
            relative_t = kf->getLoopRelativeT();//R{relo->curr}
            relative_q = (kf->getLoopRelativeQ()).toRotationMatrix();
            w_P_cur = w_R_old * relative_t + w_P_old;
            w_R_cur = w_R_old * relative_q; //R{w1->curr} = R{w1->relo}*R{relo->curr}
            double shift_yaw;
            Matrix3d shift_r;
            Vector3d shift_t; 
            shift_yaw = Utility::R2ypr(w_R_cur).x() - Utility::R2ypr(vio_R_cur).x();
            shift_r = Utility::ypr2R(Vector3d(shift_yaw, 0, 0)); //shift_r为世界系w1到w2的旋转矩阵 (hansry)
            shift_t = w_P_cur - w_R_cur * vio_R_cur.transpose() * vio_P_cur;  //shift_t为世界系w1到w2的平移矩阵 (hansry)
