...
//将关键帧中某个特征点的描述子与回环帧的所有描述子匹配，通过汉明距离从回环帧中描述子中找到最佳描述子
                         ...
                         Eigen::Vector3d &PnP_T_old, Eigen::Matrix3d &PnP_R_old)
{
    ...
    Matrix3d R_inital;
    Vector3d P_inital;
    Matrix3d R_w_c = origin_vio_R * qic;//Rwc = Rwi * Ric
    Vector3d T_w_c = origin_vio_T + origin_vio_R * tic; //Twc = Twi + Rwi * Tic

    R_inital = R_w_c.inverse(); //Rcw
    P_inital = -(R_inital * T_w_c); //Tcw
    ...
    cv::cv2eigen(r, R_pnp);
    R_w_c_old = R_pnp.transpose(); //Rwc_old = Rcw_old.transpose()
    Vector3d T_pnp, T_w_c_old;
    cv::cv2eigen(t, T_pnp);
    T_w_c_old = R_w_c_old * (-T_pnp); //Twc_old = -Rwc_old * Tcw_old

    PnP_R_old = R_w_c_old * qic.transpose(); //从相机转换到IMU　Rwi_old = Rwc_old * Rci
    PnP_T_old = T_w_c_old - PnP_R_old * tic; //Twi_old = Twc_old  - Rwi_old * Tic
...
bool KeyFrame::findConnection(KeyFrame* old_kf)
{
	...
	reduceVector(matched_2d_cur_norm, status);//norm指归一化的坐标
	...
	*/
	...
	if ((int)matched_2d_cur.size() > MIN_LOOP_NUM)
	{
	    relative_t = PnP_R_old.transpose() * (origin_vio_T - PnP_T_old); //Tio = Riw_old * (Two - Twi_old) 
	    relative_q = PnP_R_old.transpose() * origin_vio_R; //Rio = Riw_old * Rwo 
	    ...
	    if (abs(relative_yaw) < 30.0 && relative_t.norm() < 20.0)
	    {
	    	...
	    	if(FAST_RELOCALIZATION)
	    	{
				...
				//points里是匹配到的角点的归一化坐标和该地图点的id
				//channels是回环帧的pose (hansry)
