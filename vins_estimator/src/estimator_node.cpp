...
getMeasurements()
{
    ...
    while (true)
    {
        ...
        //对齐标准1：IMU最后一个数据的时间要大于第一个图像特征数据的时间
        ...
        //对齐标准2：IMU第一个数据的时间要小于第一个图像特征数据的时间
...
//基于世界坐标系下的IMU离散形式运动模型,中值积分来计算PVQ (hansry)
void imu_callback(const sensor_msgs::ImuConstPtr &imu_msg)
{
    ...
    last_imu_t = imu_msg->header.stamp.toSec();
    {
        //构造互斥锁m_state，析构时解锁，lock_guard在作用域块区间生效
...
void process()
{
    while (true)
    {
        ...
        //当meansurements为空时，调用con.wait()堵住当前线程，让该线程休眠，然后等待上面两个接收数据完成调用notify_one()唤醒当前线程
        ...
        for (auto &measurement : measurements)
        {
            ...
            for (auto &imu_msg : measurement.first)
            {
                ...
                { 
                    if (current_time < 0)
                        current_time = t;
                    double dt = t - current_time;//dt为当前时刻采样IMU数据的时间减去上次采样IMU数据的时间
                    ROS_ASSERT(dt >= 0);
                    current_time = t;
            ...
            //建立每个特征点的[feature_id, (camera_id,[x,y,z,u,v,vx,vy])]s的map
            map<int, vector<pair<int, Eigen::Matrix<double, 7, 1>>>> image;
            for (unsigned int i = 0; i < img_msg->points.size(); i++)
            {
                //表示第i个特征点的ID值
                int v = img_msg->channels[0].values[i] + 0.5; //+0.5主要为了四舍五入
                int feature_id = v / NUM_OF_CAM;
                int camera_id = v % NUM_OF_CAM; //若NUM_OF_CAM=1，则camera_id=0 (hansry)
            ...
            header.frame_id = "world"; //其中herder.frame_id表示坐标系id
...
int main(int argc, char **argv)
{
    ...
    //订阅IMU(IMU的角速度和线加速度)、feature(图像跟踪的特征点)、restart(复位信号)、match_points(重定位的匹配点)的topic,执行各自回调函数
