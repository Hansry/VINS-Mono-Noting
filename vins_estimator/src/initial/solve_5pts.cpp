...
bool MotionEstimator::solveRelativeRT(const vector<pair<Vector3d, Vector3d>> &corres, Matrix3d &Rotation, Vector3d &Translation)
{
    if (corres.size() >= 15)
    {
         ...
         *  ) 
        */   
        //可能是由于ll,rr是归一化的点，所以求出来的矩阵是Essential矩阵,若ll,rr是像素上的点，则求出来是Fundamental矩阵
        // x_{2}^{T}Ex_{1}=p_{2}^{T}Fp_{1}=0, F = K^{-T}EK^{-1} (hansry)
        ...
        */
        ...
        //(这里跟我平常的说法不太一致，我更偏向于说第l帧(参考帧)的坐标系到窗口最后一帧(当前帧)的坐标变换Rt, hansry)
