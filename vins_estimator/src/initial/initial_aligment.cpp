...
void solveGyroscopeBias(map<double, ImageFrame> &all_image_frame, Vector3d* Bgs)
{
    ...
    //将所有与图像帧对应的IMU预积分对bias的雅克比，进行Ax=b方程的构建（hansry）
    for (frame_i = all_image_frame.begin(); next(frame_i) != all_image_frame.end(); frame_i++)
    {
        ...
        //      = 2 * (r^bk_bk+1)^-1 * q_ij　其中r^bk_bk+1为IMU预积分得到俩帧之间的旋转，q_ij为相机得到的俩帧之间的旋转
        tmp_b = 2 * (frame_j->second.pre_integration->delta_q.inverse() * q_ij).vec();//只考虑虚部，实部均为１可以不考虑
        //tmp_A * delta_bg = tmp_b
        A += tmp_A.transpose() * tmp_A;//将左边转为对称矩阵，这样就可以用LDLT进行分解了
    ...
    for (int i = 0; i <= WINDOW_SIZE; i++){
        Bgs[i] += delta_bg;
    //   cout << "Bgs" << "[" << i << "]: " << Bgs[i] << endl;
    }
    for (frame_i = all_image_frame.begin(); next(frame_i) != all_image_frame.end( ); frame_i++)
    {
        frame_j = next(frame_i);
        //进行陀螺仪偏置的估计后，进行预积分的重新传播（hansry）
...
MatrixXd TangentBasis(Vector3d &g0)
{
    ...
    if(a == tmp)
        tmp << 1, 0, 0;
    //b = g^- x tmp,  tmp=(g^-==(0,0,1)?(1,0,0),(0,0,1))　(hansry)
...
{
    //g0 = (g^-)*||g||　（g^-）为得到的重力向量初始值的单位向量，||g||为重力加速度模长
...
bool LinearAlignment(map<double, ImageFrame> &all_image_frame, Vector3d &g, VectorXd &x)
{
    ...
    //矩阵A的构建具体情况见https://blog.csdn.net/Hansry/article/details/104365306
    MatrixXd A{n_state, n_state};
    A.setZero();
    VectorXd b{n_state};
    b.setZero();
    ...
    for (frame_i = all_image_frame.begin(); next(frame_i) != all_image_frame.end(); frame_i++, i++)
    {
        ...
        //这里需要注意的是由于frame_i->second.R表示的是第l帧（我们看做是第c0帧）到imu body坐标系的变换,即R^co_bk，因此需要加transpose() (hansry)
        tmp_A.block<3, 3>(0, 0) = -dt * Matrix3d::Identity();
        tmp_A.block<3, 3>(0, 6) = frame_i->second.R.transpose() * dt * dt / 2 * Matrix3d::Identity();
        //除以100意味着尺度这个变量对残差的影响力减弱了100倍,但是会导致估计的尺度比实际大100倍（hansry）
    ...
    //优化后的尺度会比实际的大100，得到后要再除以100，这么做的目的应该是要让尺度的精度更高（hansry）
