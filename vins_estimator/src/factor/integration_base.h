...
class IntegrationBase
{
  public:
...

    {
        ...
        //其中ACC_N、ACC_W、GYR_N和GYR_W分别为加速度计和陀螺仪噪声和随机偏置标准差
    ...
    /**
    * @brief   IMU预积分中采用中值积分构造误差的线性化递推方程，得到Jacobian和Covariance递推公式-> Paper 式9、10、11
    * @return  void
    */
                            ...
                            Eigen::Vector3d &result_linearized_ba, Eigen::Vector3d &result_linearized_bg, bool update_jacobian)
    {
        ...
        //IMU的中值积分计算PVQ (hansry)
        Vector3d un_acc_0 = delta_q * (_acc_0 - linearized_ba);
        Vector3d un_gyr = 0.5 * (_gyr_0 + _gyr_1) - linearized_bg;
        //Rk->i+1 = Rk->i * Ri->i+1 (hansry)
    ...
    */
    void propagate(double _dt, const Eigen::Vector3d &_acc_1, const Eigen::Vector3d &_gyr_1)
    {
        ...
        //对于每次预积分后都要进行F矩阵和V矩阵的计算,构建预积分误差的线性化递推方程，得到Jacobian和Covariance递推公式(hansry)
                                          ...
                                          const Eigen::Vector3d &Pj, const Eigen::Quaterniond &Qj, const Eigen::Vector3d &Vj, const Eigen::Vector3d &Baj, const Eigen::Vector3d &Bgj)
    {
        ...
        //O_P = 0,O_R = 3,O_V = 6,O_BA = 9,O_BG = 12（在parameters.h中进行定义） (hansry)
        //这一部分表示预积分量在i时刻下对bias的一阶泰勒展开近似，然后通过雅克比计算j时刻的PVQ预积分量而不是进行传播
        //dp_dba表示i时刻预积分量Position对IMU加速度计bias的雅克比矩阵
        //dp_dbg表示i时刻预积分量position对IMU陀螺仪bias的雅克比矩阵
        Eigen::Matrix3d dp_dba = jacobian.block<3, 3>(O_P, O_BA); 
        Eigen::Matrix3d dp_dbg = jacobian.block<3, 3>(O_P, O_BG);
        
        //dq_dgb表示i时刻预积分量Quaternion对IMU陀螺仪bias的雅克比矩阵
        Eigen::Matrix3d dq_dbg = jacobian.block<3, 3>(O_R, O_BG);
        
        //dv_dba表示i时刻预计分量Velocity对IMU加速度计bias的雅克比矩阵
        //dv_dbg表示i时刻预计分量Velocity对IMU陀螺仪bias的雅克比矩阵
        Eigen::Matrix3d dv_dba = jacobian.block<3, 3>(O_V, O_BA);
        Eigen::Matrix3d dv_dbg = jacobian.block<3, 3>(O_V, O_BG);
        
        //linearized_ba和linearized_bg为中值积分得到的偏置
        Eigen::Vector3d dba = Bai - linearized_ba; //delta ba
        Eigen::Vector3d dbg = Bgi - linearized_bg; //delta bg

        //通过一阶泰勒展开近似求得的j时刻下的预积分量 
