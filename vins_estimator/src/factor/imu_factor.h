...
class IMUFactor : public ceres::SizedCostFunction<15, 7, 9, 7, 9>
{
  public:
    ...
    virtual bool Evaluate(double const *const *parameters, double *residuals, double **jacobians) const
    {
        ...
        //Pi和Qi分别表示i时刻世界坐标系下到IMU body坐标系的平移和旋转矩阵 (hansry)
        Eigen::Vector3d Pi(parameters[0][0], parameters[0][1], parameters[0][2]);
        Eigen::Quaterniond Qi(parameters[0][6], parameters[0][3], parameters[0][4], parameters[0][5]);

        //Vi表示i时刻世界坐标系下body坐标系的速度
        Eigen::Vector3d Vi(parameters[1][0], parameters[1][1], parameters[1][2]);
        Eigen::Vector3d Bai(parameters[1][3], parameters[1][4], parameters[1][5]);
        Eigen::Vector3d Bgi(parameters[1][6], parameters[1][7], parameters[1][8]);

        //Pj和Qj分别表示j时刻世界坐标系到IMU body坐标系的平移和旋转矩阵 (hansry)
        Eigen::Vector3d Pj(parameters[2][0], parameters[2][1], parameters[2][2]);
        Eigen::Quaterniond Qj(parameters[2][6], parameters[2][3], parameters[2][4], parameters[2][5]);
        
        //Vi表示j时刻世界坐标系下body坐标系的速度
...
//delta_q = Qi.inverse() * Qj;
        ...
        // 对信息矩阵进行LLT分解，residual 还需乘以信息矩阵的sqrt_info
        ...
        if (jacobians)
        {
            ...
            //表示预积分量PVQ在i时刻下对加速度计和陀螺仪bias的雅克比矩阵
