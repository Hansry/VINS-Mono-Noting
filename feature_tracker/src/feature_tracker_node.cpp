...
void img_callback(const sensor_msgs::ImageConstPtr &img_msg)
{
    ...
    // 当前图像帧的时间与上一图像帧的时间相差1s或者当前图像帧的时间小于上一个图像帧的时间，则相机数据流不稳定
    ...
    // 并不是每读入一帧图像，就要发布特征点
    // 判断间隔时间内的发布次数
    // 当发布频率小于或者等于设定的频率FREQ时，才有可能进行发布，每发布一次，pub_count++ (hansry)
    ...
    if (img_msg->encoding == "8UC1")
    {
        ...
        img.is_bigendian = img_msg->is_bigendian;//从低地址到高地址的顺序存放数据的高位字节到低位字节
        img.step = img_msg->step;
        img.data = img_msg->data;
        img.encoding = "mono8";
        //将sensor_msgs::Image类型的图像数据拷贝给cv_bridge::CvImageConstPtr,
        //通过cv_bridge实现ros和opencv图像的传递
   ...
   if (PUB_THIS_FRAME) //对当前帧进行发布
   {
        ...
        //feature_points->channels为sensor_msgs::ChannelFloat32类型的数组
...
int main(int argc, char **argv)
{
    ...
    //通过句柄设定“~”指定命名空间为/node_name，如n.advertise<std_msgs::Bool>("restart",1000)发布的topic为/feature_tracker/restart
    ros::NodeHandle n("~");
